...
def generate_svd_file(input_filename: str, output_filename: str):
    """Generate SVD file for device family using tixml2svd
       ...
       of the PACs. Many devices in the connectivity family are similar and thus
...
def svd2rust(svd_file_path: str, device_name: str):
    ...
    Args:
        ...
        device_name (str): Device name to be used to name the PAC
    """

    # We don't want to run cargo init if the pac already exists
    if not os.path.exists("Cargo.toml"):
        subprocess.run(
            ["cargo", "init", "--lib", "--vcs=none", f"--name={device_name}_pac"]
        )
...
def add_ccfg(ccfg_path: str):
    """Add the CCFG definition to the generated PAC

       The CCFG is not really a register bank so cannot be generated by SVD2Rust.
       It is instead a specific region of flash that is a bunch of packed bitfields.

       This repo contains handmade CCFG definitions in rust.
    ...
    """
    lib_rs_path = Path("src/lib.rs").as_posix()
    with open(lib_rs_path, mode="a") as lib_file:
        # Add the CCFG module definition to the lib.rs
        lib_file.write('#[doc = "Customer Configuration Area (CCFG)"]\npub mod ccfg;\n')

    shutil.copy(ccfg_path, f"{os.curdir}/src")
    # Rename the file to ccfg.rs
    os.rename(f"{os.curdir}/src/{Path(ccfg_path).name}", f"{os.curdir}/src/ccfg.rs")
...
def app():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Rust Peripheral Access Crates for TI's SimpleLink Devices"
    )
    ...
    if not devs:
        print(
            "No device families found in the input folder, remember to point to devices folder."
        )
        return

    print("Generating PACs for TI SimpleLink devices")
    for dev in devs:
        ...
        device_x_path = Path(
            f"{old_path}/{args.input}/{device_name}_device.x"
        ).as_posix()
        patch_vectors(pac_directory, int_path, device_x_path)

        # Add the CCFG definition which is manually maintained
        ccfg_path = Path(f"{old_path}/{args.input}/{device_name}_ccfg.rs").as_posix()
        add_ccfg(ccfg_path)
