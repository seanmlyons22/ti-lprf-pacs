...
#[doc = "Field `STARTUPTIME` writer - 30:16\\]
HFXT startup time Can be used by software to plan starting HFXT ahead in time. Measured whenever HFXT is enabled in CLKULL periods (24MHz), from HFXTCTL.EN until the clock is good for radio operation (amplitude compensation is settled)."]
pub type StartuptimeW<'a, REG> = crate::FieldWriter<'a, REG, 15, u16>;
...
HFXT clock available. The frequency is not necessarily good enough for radio operation."]
    #[inline(always)]
    #[must_use]
    pub fn good(&mut self) -> GoodW<HfxtstatSpec> {
        GoodW::new(self, 0)
    }
    #[doc = "Bit 1 - 1:1\\]
HFXT clock fault Indicates a lower than expected HFXT frequency. HFXT will not recover from this fault, disabling and re-enabling HFXT is required."]
    #[inline(always)]
    #[must_use]
    pub fn fault(&mut self) -> FaultW<HfxtstatSpec> {
        FaultW::new(self, 1)
    }
    #[doc = "Bits 2:15 - 15:2\\]
Software should not rely on the value of a reserved. Writing any other value than the reset value may result in undefined behavior."]
    #[inline(always)]
    #[must_use]
    pub fn reserved2(&mut self) -> Reserved2W<HfxtstatSpec> {
...
HFXT startup time Can be used by software to plan starting HFXT ahead in time. Measured whenever HFXT is enabled in CLKULL periods (24MHz), from HFXTCTL.EN until the clock is good for radio operation (amplitude compensation is settled)."]
    #[inline(always)]
    #[must_use]
    pub fn startuptime(&mut self) -> StartuptimeW<HfxtstatSpec> {
        StartuptimeW::new(self, 16)
    }
    #[doc = "Bit 31 - 31:31\\]
Software should not rely on the value of a reserved. Writing any other value than the reset value may result in undefined behavior."]
    #[inline(always)]
    #[must_use]
    pub fn reserved31(&mut self) -> Reserved31W<HfxtstatSpec> {
