...
This bit indicates whether an entry is present at this location in the ROM table. 0x0:Rom table entry is not present and must be skipped. 0x1:Rom table entry is present."]
    #[inline(always)]
    #[must_use]
    pub fn entry_present(&mut self) -> EntryPresentW<EntryScsSpec> {
...
Base address of the highest 4KB block for the component, relative to the ROM address."]
    #[inline(always)]
    #[must_use]
    pub fn format(&mut self) -> FormatW<EntryScsSpec> {
        FormatW::new(self, 1)
    }
    #[doc = "Bits 2:11 - 11:2\\]
Software should not rely on the value of a reserved. Writing any other value than the reset value may result in undefined behavior."]
    #[inline(always)]
    #[must_use]
    pub fn reserved2(&mut self) -> Reserved2W<EntryScsSpec> {
...
Base address of the highest 4KB block for the component, relative to the ROM address."]
    #[inline(always)]
    #[must_use]
    pub fn address_offset(&mut self) -> AddressOffsetW<EntryScsSpec> {
