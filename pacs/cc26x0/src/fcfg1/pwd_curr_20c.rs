...
Worst-case baseline maximum powerdown current, in units of 0.5uA"]
    #[inline(always)]
    #[must_use]
    pub fn baseline(&mut self) -> BaselineW<PwdCurr20cSpec> {
...
Additional maximum current, in units of 1uA, with XOSC_HF on in low-power mode"]
    #[inline(always)]
    #[must_use]
    pub fn delta_xosc_lpm(&mut self) -> DeltaXoscLpmW<PwdCurr20cSpec> {
...
Additional maximum current, in 1uA units, with RF memory retention"]
    #[inline(always)]
    #[must_use]
    pub fn delta_rfmem_ret(&mut self) -> DeltaRfmemRetW<PwdCurr20cSpec> {
...
Additional maximum current, in units of 1uA, with cache retention"]
    #[inline(always)]
    #[must_use]
    pub fn delta_cache_ref(&mut self) -> DeltaCacheRefW<PwdCurr20cSpec> {
