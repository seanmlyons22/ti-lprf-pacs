...
For register NVIC_IPR*9, the priority of interrupt number 4*9+0, or is RES0 if the PE does not implement this interrupt"]
    #[inline(always)]
    #[must_use]
    pub fn pri_n0(&mut self) -> PriN0W<Ipr9Spec> {
        PriN0W::new(self, 0)
    }
    #[doc = "Bits 8:15 - 15:8\\]
For register NVIC_IPR*9, the priority of interrupt number 4*9+1, or is RES0 if the PE does not implement this interrupt"]
    #[inline(always)]
    #[must_use]
    pub fn pri_n1(&mut self) -> PriN1W<Ipr9Spec> {
        PriN1W::new(self, 8)
    }
    #[doc = "Bits 16:23 - 23:16\\]
For register NVIC_IPR*9, the priority of interrupt number 4*9+2, or is RES0 if the PE does not implement this interrupt"]
    #[inline(always)]
    #[must_use]
    pub fn pri_n2(&mut self) -> PriN2W<Ipr9Spec> {
        PriN2W::new(self, 16)
    }
    #[doc = "Bits 24:31 - 31:24\\]
For register NVIC_IPR*9, the priority of interrupt number 4*9+3, or is RES0 if the PE does not implement this interrupt"]
    #[inline(always)]
    #[must_use]
    pub fn pri_n3(&mut self) -> PriN3W<Ipr9Spec> {
        PriN3W::new(self, 24)
    }
}
