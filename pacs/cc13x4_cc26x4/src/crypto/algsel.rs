...
pub type HASH_SHA_512_R = crate::BitReader<bool>;
#[doc = "Field `HASH_SHA_512` writer - 3:3\\]
If set to one, selects the Hash engine as destination for the DMA. The maximum transfer size to DMA engine is set to 128 bytes for reading and 64 bytes for writing (the latter is only applicable if the hash result is written out via DMA)."]
pub type HASH_SHA_512_W<'a, const O: u8> = crate::BitWriter<'a, u32, ALGSEL_SPEC, bool, O>;
...
If set to one, selects the Hash engine as destination for the DMA. The maximum transfer size to DMA engine is set to 128 bytes for reading and 64 bytes for writing (the latter is only applicable if the hash result is written out via DMA)."]
    ...
    pub fn hash_sha_512(&self) -> HASH_SHA_512_R {
        HASH_SHA_512_R::new(((self.bits >> 3) & 1) != 0)
    }
    #[doc = "Bits 4:30 - 30:4\\]
Software should not rely on the value of a reserved. Writing any other value than the reset value may result in undefined behavior."]
    #[inline(always)]
...
If set to one, selects the Hash engine as destination for the DMA. The maximum transfer size to DMA engine is set to 128 bytes for reading and 64 bytes for writing (the latter is only applicable if the hash result is written out via DMA)."]
    #[inline(always)]
    #[must_use]
    pub fn hash_sha_512(&mut self) -> HASH_SHA_512_W<3> {
        HASH_SHA_512_W::new(self)
    }
    #[doc = "Bits 4:30 - 30:4\\]
Software should not rely on the value of a reserved. Writing any other value than the reset value may result in undefined behavior."]
    #[inline(always)]
