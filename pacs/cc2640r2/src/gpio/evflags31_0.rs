#[doc = "Register `EVFLAGS31_0` reader"]
pub type R = crate::R<Evflags31_0Spec>;
#[doc = "Register `EVFLAGS31_0` writer"]
pub type W = crate::W<Evflags31_0Spec>;
#[doc = "Field `DIO0` reader - 0:0\\]
Event for DIO 0"]
pub type Dio0R = crate::BitReader;
#[doc = "Field `DIO0` writer - 0:0\\]
Event for DIO 0"]
pub type Dio0W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO1` reader - 1:1\\]
Event for DIO 1"]
pub type Dio1R = crate::BitReader;
#[doc = "Field `DIO1` writer - 1:1\\]
Event for DIO 1"]
pub type Dio1W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO2` reader - 2:2\\]
Event for DIO 2"]
pub type Dio2R = crate::BitReader;
#[doc = "Field `DIO2` writer - 2:2\\]
Event for DIO 2"]
pub type Dio2W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO3` reader - 3:3\\]
Event for DIO 3"]
pub type Dio3R = crate::BitReader;
#[doc = "Field `DIO3` writer - 3:3\\]
Event for DIO 3"]
pub type Dio3W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO4` reader - 4:4\\]
Event for DIO 4"]
pub type Dio4R = crate::BitReader;
#[doc = "Field `DIO4` writer - 4:4\\]
Event for DIO 4"]
pub type Dio4W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO5` reader - 5:5\\]
Event for DIO 5"]
pub type Dio5R = crate::BitReader;
#[doc = "Field `DIO5` writer - 5:5\\]
Event for DIO 5"]
pub type Dio5W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO6` reader - 6:6\\]
Event for DIO 6"]
pub type Dio6R = crate::BitReader;
#[doc = "Field `DIO6` writer - 6:6\\]
Event for DIO 6"]
pub type Dio6W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO7` reader - 7:7\\]
Event for DIO 7"]
pub type Dio7R = crate::BitReader;
#[doc = "Field `DIO7` writer - 7:7\\]
Event for DIO 7"]
pub type Dio7W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO8` reader - 8:8\\]
Event for DIO 8"]
pub type Dio8R = crate::BitReader;
#[doc = "Field `DIO8` writer - 8:8\\]
Event for DIO 8"]
pub type Dio8W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO9` reader - 9:9\\]
Event for DIO 9"]
pub type Dio9R = crate::BitReader;
#[doc = "Field `DIO9` writer - 9:9\\]
Event for DIO 9"]
pub type Dio9W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO10` reader - 10:10\\]
Event for DIO 10"]
pub type Dio10R = crate::BitReader;
#[doc = "Field `DIO10` writer - 10:10\\]
Event for DIO 10"]
pub type Dio10W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO11` reader - 11:11\\]
Event for DIO 11"]
pub type Dio11R = crate::BitReader;
#[doc = "Field `DIO11` writer - 11:11\\]
Event for DIO 11"]
pub type Dio11W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO12` reader - 12:12\\]
Event for DIO 12"]
pub type Dio12R = crate::BitReader;
#[doc = "Field `DIO12` writer - 12:12\\]
Event for DIO 12"]
pub type Dio12W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO13` reader - 13:13\\]
Event for DIO 13"]
pub type Dio13R = crate::BitReader;
#[doc = "Field `DIO13` writer - 13:13\\]
Event for DIO 13"]
pub type Dio13W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO14` reader - 14:14\\]
Event for DIO 14"]
pub type Dio14R = crate::BitReader;
#[doc = "Field `DIO14` writer - 14:14\\]
Event for DIO 14"]
pub type Dio14W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO15` reader - 15:15\\]
Event for DIO 15"]
pub type Dio15R = crate::BitReader;
#[doc = "Field `DIO15` writer - 15:15\\]
Event for DIO 15"]
pub type Dio15W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO16` reader - 16:16\\]
Event for DIO 16"]
pub type Dio16R = crate::BitReader;
#[doc = "Field `DIO16` writer - 16:16\\]
Event for DIO 16"]
pub type Dio16W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO17` reader - 17:17\\]
Event for DIO 17"]
pub type Dio17R = crate::BitReader;
#[doc = "Field `DIO17` writer - 17:17\\]
Event for DIO 17"]
pub type Dio17W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO18` reader - 18:18\\]
Event for DIO 18"]
pub type Dio18R = crate::BitReader;
#[doc = "Field `DIO18` writer - 18:18\\]
Event for DIO 18"]
pub type Dio18W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO19` reader - 19:19\\]
Event for DIO 19"]
pub type Dio19R = crate::BitReader;
#[doc = "Field `DIO19` writer - 19:19\\]
Event for DIO 19"]
pub type Dio19W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO20` reader - 20:20\\]
Event for DIO 20"]
pub type Dio20R = crate::BitReader;
#[doc = "Field `DIO20` writer - 20:20\\]
Event for DIO 20"]
pub type Dio20W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO21` reader - 21:21\\]
Event for DIO 21"]
pub type Dio21R = crate::BitReader;
#[doc = "Field `DIO21` writer - 21:21\\]
Event for DIO 21"]
pub type Dio21W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO22` reader - 22:22\\]
Event for DIO 22"]
pub type Dio22R = crate::BitReader;
#[doc = "Field `DIO22` writer - 22:22\\]
Event for DIO 22"]
pub type Dio22W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO23` reader - 23:23\\]
Event for DIO 23"]
pub type Dio23R = crate::BitReader;
#[doc = "Field `DIO23` writer - 23:23\\]
Event for DIO 23"]
pub type Dio23W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO24` reader - 24:24\\]
Event for DIO 24"]
pub type Dio24R = crate::BitReader;
#[doc = "Field `DIO24` writer - 24:24\\]
Event for DIO 24"]
pub type Dio24W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO25` reader - 25:25\\]
Event for DIO 25"]
pub type Dio25R = crate::BitReader;
#[doc = "Field `DIO25` writer - 25:25\\]
Event for DIO 25"]
pub type Dio25W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO26` reader - 26:26\\]
Event for DIO 26"]
pub type Dio26R = crate::BitReader;
#[doc = "Field `DIO26` writer - 26:26\\]
Event for DIO 26"]
pub type Dio26W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO27` reader - 27:27\\]
Event for DIO 27"]
pub type Dio27R = crate::BitReader;
#[doc = "Field `DIO27` writer - 27:27\\]
Event for DIO 27"]
pub type Dio27W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO28` reader - 28:28\\]
Event for DIO 28"]
pub type Dio28R = crate::BitReader;
#[doc = "Field `DIO28` writer - 28:28\\]
Event for DIO 28"]
pub type Dio28W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO29` reader - 29:29\\]
Event for DIO 29"]
pub type Dio29R = crate::BitReader;
#[doc = "Field `DIO29` writer - 29:29\\]
Event for DIO 29"]
pub type Dio29W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO30` reader - 30:30\\]
Event for DIO 30"]
pub type Dio30R = crate::BitReader;
#[doc = "Field `DIO30` writer - 30:30\\]
Event for DIO 30"]
pub type Dio30W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `DIO31` reader - 31:31\\]
Event for DIO 31"]
pub type Dio31R = crate::BitReader;
#[doc = "Field `DIO31` writer - 31:31\\]
Event for DIO 31"]
pub type Dio31W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    #[doc = "Bit 0 - 0:0\\]
Event for DIO 0"]
    #[inline(always)]
    pub fn dio0(&self) -> Dio0R {
        Dio0R::new((self.bits & 1) != 0)
    }
    #[doc = "Bit 1 - 1:1\\]
Event for DIO 1"]
    #[inline(always)]
    pub fn dio1(&self) -> Dio1R {
        Dio1R::new(((self.bits >> 1) & 1) != 0)
    }
    #[doc = "Bit 2 - 2:2\\]
Event for DIO 2"]
    #[inline(always)]
    pub fn dio2(&self) -> Dio2R {
        Dio2R::new(((self.bits >> 2) & 1) != 0)
    }
    #[doc = "Bit 3 - 3:3\\]
Event for DIO 3"]
    #[inline(always)]
    pub fn dio3(&self) -> Dio3R {
        Dio3R::new(((self.bits >> 3) & 1) != 0)
    }
    #[doc = "Bit 4 - 4:4\\]
Event for DIO 4"]
    #[inline(always)]
    pub fn dio4(&self) -> Dio4R {
        Dio4R::new(((self.bits >> 4) & 1) != 0)
    }
    #[doc = "Bit 5 - 5:5\\]
Event for DIO 5"]
    #[inline(always)]
    pub fn dio5(&self) -> Dio5R {
        Dio5R::new(((self.bits >> 5) & 1) != 0)
    }
    #[doc = "Bit 6 - 6:6\\]
Event for DIO 6"]
    #[inline(always)]
    pub fn dio6(&self) -> Dio6R {
        Dio6R::new(((self.bits >> 6) & 1) != 0)
    }
    #[doc = "Bit 7 - 7:7\\]
Event for DIO 7"]
    #[inline(always)]
    pub fn dio7(&self) -> Dio7R {
        Dio7R::new(((self.bits >> 7) & 1) != 0)
    }
    #[doc = "Bit 8 - 8:8\\]
Event for DIO 8"]
    #[inline(always)]
    pub fn dio8(&self) -> Dio8R {
        Dio8R::new(((self.bits >> 8) & 1) != 0)
    }
    #[doc = "Bit 9 - 9:9\\]
Event for DIO 9"]
    #[inline(always)]
    pub fn dio9(&self) -> Dio9R {
        Dio9R::new(((self.bits >> 9) & 1) != 0)
    }
    #[doc = "Bit 10 - 10:10\\]
Event for DIO 10"]
    #[inline(always)]
    pub fn dio10(&self) -> Dio10R {
        Dio10R::new(((self.bits >> 10) & 1) != 0)
    }
    #[doc = "Bit 11 - 11:11\\]
Event for DIO 11"]
    #[inline(always)]
    pub fn dio11(&self) -> Dio11R {
        Dio11R::new(((self.bits >> 11) & 1) != 0)
    }
    #[doc = "Bit 12 - 12:12\\]
Event for DIO 12"]
    #[inline(always)]
    pub fn dio12(&self) -> Dio12R {
        Dio12R::new(((self.bits >> 12) & 1) != 0)
    }
    #[doc = "Bit 13 - 13:13\\]
Event for DIO 13"]
    #[inline(always)]
    pub fn dio13(&self) -> Dio13R {
        Dio13R::new(((self.bits >> 13) & 1) != 0)
    }
    #[doc = "Bit 14 - 14:14\\]
Event for DIO 14"]
    #[inline(always)]
    pub fn dio14(&self) -> Dio14R {
        Dio14R::new(((self.bits >> 14) & 1) != 0)
    }
    #[doc = "Bit 15 - 15:15\\]
Event for DIO 15"]
    #[inline(always)]
    pub fn dio15(&self) -> Dio15R {
        Dio15R::new(((self.bits >> 15) & 1) != 0)
    }
    #[doc = "Bit 16 - 16:16\\]
Event for DIO 16"]
    #[inline(always)]
    pub fn dio16(&self) -> Dio16R {
        Dio16R::new(((self.bits >> 16) & 1) != 0)
    }
    #[doc = "Bit 17 - 17:17\\]
Event for DIO 17"]
    #[inline(always)]
    pub fn dio17(&self) -> Dio17R {
        Dio17R::new(((self.bits >> 17) & 1) != 0)
    }
    #[doc = "Bit 18 - 18:18\\]
Event for DIO 18"]
    #[inline(always)]
    pub fn dio18(&self) -> Dio18R {
        Dio18R::new(((self.bits >> 18) & 1) != 0)
    }
    #[doc = "Bit 19 - 19:19\\]
Event for DIO 19"]
    #[inline(always)]
    pub fn dio19(&self) -> Dio19R {
        Dio19R::new(((self.bits >> 19) & 1) != 0)
    }
    #[doc = "Bit 20 - 20:20\\]
Event for DIO 20"]
    #[inline(always)]
    pub fn dio20(&self) -> Dio20R {
        Dio20R::new(((self.bits >> 20) & 1) != 0)
    }
    #[doc = "Bit 21 - 21:21\\]
Event for DIO 21"]
    #[inline(always)]
    pub fn dio21(&self) -> Dio21R {
        Dio21R::new(((self.bits >> 21) & 1) != 0)
    }
    #[doc = "Bit 22 - 22:22\\]
Event for DIO 22"]
    #[inline(always)]
    pub fn dio22(&self) -> Dio22R {
        Dio22R::new(((self.bits >> 22) & 1) != 0)
    }
    #[doc = "Bit 23 - 23:23\\]
Event for DIO 23"]
    #[inline(always)]
    pub fn dio23(&self) -> Dio23R {
        Dio23R::new(((self.bits >> 23) & 1) != 0)
    }
    #[doc = "Bit 24 - 24:24\\]
Event for DIO 24"]
    #[inline(always)]
    pub fn dio24(&self) -> Dio24R {
        Dio24R::new(((self.bits >> 24) & 1) != 0)
    }
    #[doc = "Bit 25 - 25:25\\]
Event for DIO 25"]
    #[inline(always)]
    pub fn dio25(&self) -> Dio25R {
        Dio25R::new(((self.bits >> 25) & 1) != 0)
    }
    #[doc = "Bit 26 - 26:26\\]
Event for DIO 26"]
    #[inline(always)]
    pub fn dio26(&self) -> Dio26R {
        Dio26R::new(((self.bits >> 26) & 1) != 0)
    }
    #[doc = "Bit 27 - 27:27\\]
Event for DIO 27"]
    #[inline(always)]
    pub fn dio27(&self) -> Dio27R {
        Dio27R::new(((self.bits >> 27) & 1) != 0)
    }
    #[doc = "Bit 28 - 28:28\\]
Event for DIO 28"]
    #[inline(always)]
    pub fn dio28(&self) -> Dio28R {
        Dio28R::new(((self.bits >> 28) & 1) != 0)
    }
    #[doc = "Bit 29 - 29:29\\]
Event for DIO 29"]
    #[inline(always)]
    pub fn dio29(&self) -> Dio29R {
        Dio29R::new(((self.bits >> 29) & 1) != 0)
    }
    #[doc = "Bit 30 - 30:30\\]
Event for DIO 30"]
    #[inline(always)]
    pub fn dio30(&self) -> Dio30R {
        Dio30R::new(((self.bits >> 30) & 1) != 0)
    }
    #[doc = "Bit 31 - 31:31\\]
Event for DIO 31"]
    #[inline(always)]
    pub fn dio31(&self) -> Dio31R {
        Dio31R::new(((self.bits >> 31) & 1) != 0)
    }
}
impl W {
    #[doc = "Bit 0 - 0:0\\]
Event for DIO 0"]
    #[inline(always)]
    #[must_use]
    pub fn dio0(&mut self) -> Dio0W<Evflags31_0Spec> {
        Dio0W::new(self, 0)
    }
    #[doc = "Bit 1 - 1:1\\]
Event for DIO 1"]
    #[inline(always)]
    #[must_use]
    pub fn dio1(&mut self) -> Dio1W<Evflags31_0Spec> {
        Dio1W::new(self, 1)
    }
    #[doc = "Bit 2 - 2:2\\]
Event for DIO 2"]
    #[inline(always)]
    #[must_use]
    pub fn dio2(&mut self) -> Dio2W<Evflags31_0Spec> {
        Dio2W::new(self, 2)
    }
    #[doc = "Bit 3 - 3:3\\]
Event for DIO 3"]
    #[inline(always)]
    #[must_use]
    pub fn dio3(&mut self) -> Dio3W<Evflags31_0Spec> {
        Dio3W::new(self, 3)
    }
    #[doc = "Bit 4 - 4:4\\]
Event for DIO 4"]
    #[inline(always)]
    #[must_use]
    pub fn dio4(&mut self) -> Dio4W<Evflags31_0Spec> {
        Dio4W::new(self, 4)
    }
    #[doc = "Bit 5 - 5:5\\]
Event for DIO 5"]
    #[inline(always)]
    #[must_use]
    pub fn dio5(&mut self) -> Dio5W<Evflags31_0Spec> {
        Dio5W::new(self, 5)
    }
    #[doc = "Bit 6 - 6:6\\]
Event for DIO 6"]
    #[inline(always)]
    #[must_use]
    pub fn dio6(&mut self) -> Dio6W<Evflags31_0Spec> {
        Dio6W::new(self, 6)
    }
    #[doc = "Bit 7 - 7:7\\]
Event for DIO 7"]
    #[inline(always)]
    #[must_use]
    pub fn dio7(&mut self) -> Dio7W<Evflags31_0Spec> {
        Dio7W::new(self, 7)
    }
    #[doc = "Bit 8 - 8:8\\]
Event for DIO 8"]
    #[inline(always)]
    #[must_use]
    pub fn dio8(&mut self) -> Dio8W<Evflags31_0Spec> {
        Dio8W::new(self, 8)
    }
    #[doc = "Bit 9 - 9:9\\]
Event for DIO 9"]
    #[inline(always)]
    #[must_use]
    pub fn dio9(&mut self) -> Dio9W<Evflags31_0Spec> {
        Dio9W::new(self, 9)
    }
    #[doc = "Bit 10 - 10:10\\]
Event for DIO 10"]
    #[inline(always)]
    #[must_use]
    pub fn dio10(&mut self) -> Dio10W<Evflags31_0Spec> {
        Dio10W::new(self, 10)
    }
    #[doc = "Bit 11 - 11:11\\]
Event for DIO 11"]
    #[inline(always)]
    #[must_use]
    pub fn dio11(&mut self) -> Dio11W<Evflags31_0Spec> {
        Dio11W::new(self, 11)
    }
    #[doc = "Bit 12 - 12:12\\]
Event for DIO 12"]
    #[inline(always)]
    #[must_use]
    pub fn dio12(&mut self) -> Dio12W<Evflags31_0Spec> {
        Dio12W::new(self, 12)
    }
    #[doc = "Bit 13 - 13:13\\]
Event for DIO 13"]
    #[inline(always)]
    #[must_use]
    pub fn dio13(&mut self) -> Dio13W<Evflags31_0Spec> {
        Dio13W::new(self, 13)
    }
    #[doc = "Bit 14 - 14:14\\]
Event for DIO 14"]
    #[inline(always)]
    #[must_use]
    pub fn dio14(&mut self) -> Dio14W<Evflags31_0Spec> {
        Dio14W::new(self, 14)
    }
    #[doc = "Bit 15 - 15:15\\]
Event for DIO 15"]
    #[inline(always)]
    #[must_use]
    pub fn dio15(&mut self) -> Dio15W<Evflags31_0Spec> {
        Dio15W::new(self, 15)
    }
    #[doc = "Bit 16 - 16:16\\]
Event for DIO 16"]
    #[inline(always)]
    #[must_use]
    pub fn dio16(&mut self) -> Dio16W<Evflags31_0Spec> {
        Dio16W::new(self, 16)
    }
    #[doc = "Bit 17 - 17:17\\]
Event for DIO 17"]
    #[inline(always)]
    #[must_use]
    pub fn dio17(&mut self) -> Dio17W<Evflags31_0Spec> {
        Dio17W::new(self, 17)
    }
    #[doc = "Bit 18 - 18:18\\]
Event for DIO 18"]
    #[inline(always)]
    #[must_use]
    pub fn dio18(&mut self) -> Dio18W<Evflags31_0Spec> {
        Dio18W::new(self, 18)
    }
    #[doc = "Bit 19 - 19:19\\]
Event for DIO 19"]
    #[inline(always)]
    #[must_use]
    pub fn dio19(&mut self) -> Dio19W<Evflags31_0Spec> {
        Dio19W::new(self, 19)
    }
    #[doc = "Bit 20 - 20:20\\]
Event for DIO 20"]
    #[inline(always)]
    #[must_use]
    pub fn dio20(&mut self) -> Dio20W<Evflags31_0Spec> {
        Dio20W::new(self, 20)
    }
    #[doc = "Bit 21 - 21:21\\]
Event for DIO 21"]
    #[inline(always)]
    #[must_use]
    pub fn dio21(&mut self) -> Dio21W<Evflags31_0Spec> {
        Dio21W::new(self, 21)
    }
    #[doc = "Bit 22 - 22:22\\]
Event for DIO 22"]
    #[inline(always)]
    #[must_use]
    pub fn dio22(&mut self) -> Dio22W<Evflags31_0Spec> {
        Dio22W::new(self, 22)
    }
    #[doc = "Bit 23 - 23:23\\]
Event for DIO 23"]
    #[inline(always)]
    #[must_use]
    pub fn dio23(&mut self) -> Dio23W<Evflags31_0Spec> {
        Dio23W::new(self, 23)
    }
    #[doc = "Bit 24 - 24:24\\]
Event for DIO 24"]
    #[inline(always)]
    #[must_use]
    pub fn dio24(&mut self) -> Dio24W<Evflags31_0Spec> {
        Dio24W::new(self, 24)
    }
    #[doc = "Bit 25 - 25:25\\]
Event for DIO 25"]
    #[inline(always)]
    #[must_use]
    pub fn dio25(&mut self) -> Dio25W<Evflags31_0Spec> {
        Dio25W::new(self, 25)
    }
    #[doc = "Bit 26 - 26:26\\]
Event for DIO 26"]
    #[inline(always)]
    #[must_use]
    pub fn dio26(&mut self) -> Dio26W<Evflags31_0Spec> {
        Dio26W::new(self, 26)
    }
    #[doc = "Bit 27 - 27:27\\]
Event for DIO 27"]
    #[inline(always)]
    #[must_use]
    pub fn dio27(&mut self) -> Dio27W<Evflags31_0Spec> {
        Dio27W::new(self, 27)
    }
    #[doc = "Bit 28 - 28:28\\]
Event for DIO 28"]
    #[inline(always)]
    #[must_use]
    pub fn dio28(&mut self) -> Dio28W<Evflags31_0Spec> {
        Dio28W::new(self, 28)
    }
    #[doc = "Bit 29 - 29:29\\]
Event for DIO 29"]
    #[inline(always)]
    #[must_use]
    pub fn dio29(&mut self) -> Dio29W<Evflags31_0Spec> {
        Dio29W::new(self, 29)
    }
    #[doc = "Bit 30 - 30:30\\]
Event for DIO 30"]
    #[inline(always)]
    #[must_use]
    pub fn dio30(&mut self) -> Dio30W<Evflags31_0Spec> {
        Dio30W::new(self, 30)
    }
    #[doc = "Bit 31 - 31:31\\]
Event for DIO 31"]
    #[inline(always)]
    #[must_use]
    pub fn dio31(&mut self) -> Dio31W<Evflags31_0Spec> {
        Dio31W::new(self, 31)
    }
}
#[doc = "Event Register for DIO 0 to 31 Reading this registers will return 1 for triggered event and 0 for non-triggered events. Writing a 1 to a bit field will clear the event. The configuration of events is done inside MCU IOC, e.g. events for DIO #0 is configured in IOC:IOCFG0.EDGE_DET and IOC:IOCFG0.EDGE_IRQ_EN.\n\nYou can [`read`](crate::generic::Reg::read) this register and get [`evflags31_0::R`](R).  You can [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`write_with_zero`](crate::generic::Reg::write_with_zero) this register using [`evflags31_0::W`](W). You can also [`modify`](crate::generic::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct Evflags31_0Spec;
impl crate::RegisterSpec for Evflags31_0Spec {
    type Ux = u32;
}
#[doc = "`read()` method returns [`evflags31_0::R`](R) reader structure"]
impl crate::Readable for Evflags31_0Spec {}
#[doc = "`write(|w| ..)` method takes [`evflags31_0::W`](W) writer structure"]
impl crate::Writable for Evflags31_0Spec {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
#[doc = "`reset()` method sets EVFLAGS31_0 to value 0"]
impl crate::Resettable for Evflags31_0Spec {
    const RESET_VALUE: u32 = 0;
}
